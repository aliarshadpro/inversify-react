...
interface ResolveOptionalDecorator {
  <T>(serviceIdentifier: ServiceIdentifier<T>, defaultValue?: T): (
    target: any,
    name: string,
    descriptor?: any
  ) => any;
...
interface ResolveAllDecorator {
  <T>(serviceIdentifier: ServiceIdentifier<T>): (
    target: any,
    name: string,
    descriptor?: any
  ) => any;
...
function applyResolveDecorator(
  target: any,
  name: string,
...
const resolve = <ResolveDecorator>(
...
resolve.optional = <ResolveOptionalDecorator>(
  function resolveOptional<T>(...args: unknown[]) {
...
resolve.optional.all = <ResolveAllDecorator>(
  function resolveAll<T>(...args: unknown[]) {
    if (typeof args[1] === "string" && args.length === 3) {
      const [target, name, descriptor] = args;
      const type = getDesignType(target, name);
